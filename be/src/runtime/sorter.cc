...
Sorter::Sorter(const std::vector<ScalarExpr*>& ordering_exprs,
    ...
    bool enable_spilling,bool& is_analytic_sort)
  : node_id_(node_id),
    state_(state),
    ...
    run_sizes_(NULL),
	is_analytic_sort(is_analytic_sort){
		LOG(INFO) << "new the sorter ,the  is_analytic_sort is " << is_analytic_sort;
    }
...
Status Sorter::SortCurrentInputRun() {
...

  {
   ...
   // LOG(INFO) << "sort the run ,the  is_analytic_sort is " << is_analytic_sort;
    if(is_analytic_sort){
    		unsorted_run_->set_sorted();
    }else{
