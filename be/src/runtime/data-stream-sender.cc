...
Status DataStreamSender::Channel::Init(RuntimeState* state) {
  ...
  // TODO: figure out how to size batch_
    //LOG(INFO) << "DataStreamSender::Channel::Init  row_desc_ GetRowSize  "  << row_desc_->GetRowSize()  << "   buffer_size_  "  << buffer_size_;
...
Status DataStreamSender::Channel::AddRow(TupleRow* row) {
 ...
 // LOG(INFO) << " DataStreamSender::Channel::AddRow ,the batch size is  " << batch_->num_rows();
...
Status DataStreamSender::Channel::MySend(){
	RETURN_IF_ERROR(SendCurrentBatch());
	return Status::OK();
}
...
Status DataStreamSender::Channel::SendCurrentBatch() {
  ...
  //LOG(INFO) << "  DataStreamSender::Channel::SendCurrentBatch the batch size is " << batch_.get()->num_rows();
  WaitForRpc();
...
DataStreamSender::DataStreamSender(int sender_id, const RowDescriptor* row_desc,
  ...
  for (int i = 0; i < destinations.size(); ++i) {
    //LOG(INFO) << "destinations server is  " << destinations[i].server ;
    channels_.push_back(
...
Status DataStreamSender::Prepare(RuntimeState* state, MemTracker* parent_mem_tracker) {
  ...
  //LOG(INFO) << "DataStreamSender Prepare,the  channels_ size is " << channels_.size();
  for (int i = 0; i < channels_.size(); ++i) {
...
Status DataStreamSender::Send(RuntimeState* state, RowBatch* batch) {
  //LOG(INFO) << "DataStreamSender::Send  the batch size is " << batch->num_rows() << "   partition_type_" << partition_type_ << "  channels_.size()   " << channels_.size() ;
  DCHECK(!closed_);
  DCHECK(!flushed_);
  ...
  if (partition_type_ == TPartitionType::UNPARTITIONED || channels_.size() == 1) {
    ...
    for(int i=0;i<num_channels;i++){
    			RETURN_IF_ERROR(channels_[i]->MySend());
    }
