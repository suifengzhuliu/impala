...
//LOG(INFO) << " DataStreamRecvr::SenderQueue::GetBatch  the size is " << result->num_rows();
...
void DataStreamRecvr::SenderQueue::AddBatch(const TRowBatch& thrift_batch) {
  //LOG(INFO) << "DataStreamRecvr::SenderQueue::AddBatch  thrift_batch SIZE IS " << thrift_batch.num_rows;
  unique_lock<mutex> l(lock_);
...
Status DataStreamRecvr::GetBatch(RowBatch** next_batch) {
  DCHECK(!is_merging_);
  //LOG(INFO) << "DataStreamRecvr::GetBatch  start,sender_queues_ size  " << sender_queues_.size();
