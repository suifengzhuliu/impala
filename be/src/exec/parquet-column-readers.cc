...
Status BaseScalarColumnReader::ReadDataPage() {
 ...
 LOG(INFO) << "BaseScalarColumnReader::ReadDataPage  START" << parent_->scratch_batch_->num_tuples;
    parent_->scratch_batch_->aux_mem_pool.AcquireData(data_page_pool_.get(), false);
 LOG(INFO) << "BaseScalarColumnReader::ReadDataPage  END" << parent_->scratch_batch_->num_tuples;
  } else {
