...
Status SortNode::Init(const TPlanNode& tnode, RuntimeState* state) {
  ...
  is_analytic_sort = tnode.sort_node.isAnalyticSort;
  //LOG(INFO) << "from thrift ,the  is_analytic_sort is " << is_analytic_sort;
...
Status SortNode::Prepare(RuntimeState* state) {
  ...
  // LOG(INFO) << "SortNode::Prepare ,the  is_analytic_sort is " << &is_analytic_sort << "   result is " << is_analytic_sort ;
  sorter_.reset(
      new Sorter(ordering_exprs_, is_asc_order_, nulls_first_, sort_tuple_exprs_,
          &row_descriptor_, mem_tracker(), &buffer_pool_client_,
          resource_profile_.spillable_buffer_size, runtime_profile(), state, id(), true,is_analytic_sort));
...
Status SortNode::GetNext(RuntimeState* state, RowBatch* row_batch, bool* eos) {
 //LOG(INFO) << "sort node get next ,the row batch size is " <<  row_batch->num_rows();
  SCOPED_TIMER(runtime_profile_->total_time_counter());
...
Status SortNode::SortInput(RuntimeState* state) {
  ...
  //LOG(INFO) <<  "child(0)  DEBUG INFO" << child(0)->DebugString() <<   " child num_rows_returned_ is  " << num_rows_returned_;
  bool eos;
  do {
    RETURN_IF_ERROR(child(0)->GetNext(state, &batch, &eos));
    //LOG(INFO) << "sort node  ,the batch size is " << batch.num_rows();
