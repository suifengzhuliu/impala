...
Status ExchangeNode::FillInputRowBatch(RuntimeState* state) {
 //LOG(INFO) << "ExchangeNode::FillInputRowBatch  start  input batch size is " <<  (input_batch_ != NULL ? input_batch_->num_rows() : -1);
  DCHECK(!is_merging_);
  Status ret_status;
  {
    SCOPED_TIMER(state->total_network_receive_timer());
    ret_status = stream_recvr_->GetBatch(&input_batch_);
//LOG(INFO) << "ExchangeNode::FillInputRowBatch  end  input batch size is " <<  (input_batch_ != NULL ? input_batch_->num_rows() : -1);
  }
...
Status ExchangeNode::GetNext(RuntimeState* state, RowBatch* output_batch, bool* eos) {
	//LOG(INFO) << "ExchangeNode   GetNext start ----------";
	//LOG(INFO) << "ExchangeNode::GetNext start input SIZE IS " << (input_batch_ != NULL ? input_batch_->num_rows() : -1);
	//LOG(INFO) << "  ExchangeNode::GetNext start  is_merging_  " << is_merging_;
	//LOG(INFO) << "ExchangeNode::GetNext start output_batch capacity IS " << (output_batch != NULL ? output_batch->capacity() : -1);
  ...
  RETURN_IF_ERROR(ExecDebugAction(TExecNodePhase::GETNEXT, state));
	...
	if (is_merging_)
		return GetNextMerging(state, output_batch, eos);

	SCOPED_TIMER(convert_row_batch_timer_);
	RETURN_IF_CANCELLED(state);
	RETURN_IF_ERROR(QueryMaintenance(state));

//	LOG(INFO) << "while begin,input_batch_ == NULL :  "
//			<< (input_batch_ == NULL) << " output_batch size is "
//			<< output_batch->num_rows() << " input size is "
//			<< (input_batch_ != NULL ? input_batch_->num_rows() : -1)
//			<< "  next_row_idx_ is " << next_row_idx_;
...
//		LOG(INFO) << "while end,input_batch_ == NULL :  "
//			<< (input_batch_ == NULL) << " output_batch size is "
//			<< output_batch->num_rows() << " input size is "
//			<< (input_batch_ != NULL ? input_batch_->num_rows() : -1)
//			<< "  next_row_idx_ is " << next_row_idx_;
	...
	//	LOG(INFO)
	//			<< "ExchangeNode::GetNext output_batch AtCapacity ,return , output_batch size is  "
	//			<< output_batch->num_rows() << "  num_rows_returned_  "
	//			<< num_rows_returned_;
		return Status::OK();
	}

	stream_recvr_->TransferAllResources(output_batch);
//	LOG(INFO) << " TransferAllResources end ,input_batch_ == NULL :  "
//			<< (input_batch_ == NULL) << " output_batch size is "
//			<< output_batch->num_rows() << " input size is "
//			<< (input_batch_ != NULL ? input_batch_->num_rows() : -1);

	RETURN_IF_ERROR(FillInputRowBatch(state));
//	LOG(INFO) << "FillInputRowBatch end.  output batch size is "
//			<< output_batch->num_rows() << "  input_batch_ == NULL   is :"
//			<< (input_batch_ == NULL) << "    input_batch_->num_rows()  "
//			<< (input_batch_ != NULL ? input_batch_->num_rows() : -1);

	if (input_batch_ == NULL) {
		*eos = true;
	} else {
		*eos = false;
	}
	//LOG(INFO) << "exchange getnext end";
	next_row_idx_ = 0;
	return Status::OK();
...
Status ExchangeNode::GetNextMerging(RuntimeState* state, RowBatch* output_batch,
  ...
  //LOG(INFO) << " ExchangeNode::GetNextMerging  ,the return batch size is " <<  output_batch->num_rows();
