...
Status HdfsParquetScanner::GetNextInternal(RowBatch* row_batch) {
  ...
  while (advance_row_group_ || column_readers_[0]->RowGroupAtEnd()) {
    ...
    RETURN_IF_ERROR(NextRowGroup());
 //LOG(INFO) << "  advance_row_group_  ,the scratch_batch_ size is " << scratch_batch_->num_tuples << " row_batch size    "  << row_batch->num_rows()  ;
    DCHECK_LE(row_group_idx_, file_metadata_.row_groups.size());
 ...
 //LOG(INFO) << "after AssembleRows ,the scratch_batch_ is "  << scratch_batch_->num_tuples << "  row_batch size   is " <<  row_batch->num_rows() ;
  assemble_rows_timer_.Stop();
  RETURN_IF_ERROR(status);
  if (!parse_status_.ok()) {
