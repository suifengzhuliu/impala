...
Status HdfsScanNode::GetNext(RuntimeState* state, RowBatch* row_batch, bool* eos) {
  ...
  //LOG(INFO) << "hdfs scan node ,the batch size is " << row_batch->num_rows();
  if (!status.ok() || *eos) {
...
void HdfsScanNode::ThreadTokenAvailableCb(ThreadResourceMgr::ResourcePool* pool) {
  ...
  if (!initial_ranges_issued_) return;
   ...
   //LOG(INFO) << "ThreadTokenAvailableCb  active_scanner_thread_counter_   : " << active_scanner_thread_counter_.value();
  ...
  while (true) {
   ...
   //LOG(INFO) << "scan thread name "  << name;
    ...
    scanner_threads_.AddThread(move(t));
     //LOG(INFO) << "scan thread size " << scanner_threads_.Size();
  }
}
...
//      LOG(INFO) << "GET RANGE  the len is  " << scan_range->len() << " the file name is " << scan_range->file_string() << " offset is   " << scan_range->offset();
...
Status HdfsScanNode::ProcessSplit(const vector<FilterContext>& filter_ctxs,
  ...
  //LOG(INFO) << "ProcessSplit  the len is  " << scan_range->len() << " the file name is " << scan_range->file_string() << " offset is   " << scan_range->offset();
